...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <a href={`/goods/${product.id}`} className="flex flex-col gap-y-2 w-full">
      ...
      <figcaption className="flex flex-col gap-y-1">
        ...
        <div className="flex flex-row items-center gap-x-2">
          ...
          <span className="text-[#808080] font-semibold line-through text-[14px]">500$</span>
        </div>
        <div className="flex items-center">
          {[...Array(maxStars)].map((_, index) => (
            <svg
              key={index}
              className={`w-4 h-4 me-1 ${index < product.rating ? "text-[#FFAD33]" : "text-[#BFBFBF]"}`}
              aria-hidden="true"
          ...
          <span className="text-[#808080] font-semibold text-[15px] ms-2">({product.reviews})</span>
        </div>
      </figcaption>
    </a>
  );
