...
const calculateTimeLeft = () => {
  const endTime = new Date("2024-11-16T00:00:00"); // Встановіть конкретну дату та час
...
export default function App() {
  ...
  useEffect(() => {
    const timerVisibility = setTimeout(() => {
      setIsVisible(true);
    }, 1);

    const countdownTimer = setInterval(() => {
      setTimer(calculateTimeLeft());
    }, 1000); // Оновлюємо таймер щосекунди
        ...
        >
          <div ref={sliderRef} className={`${styles.keenSlider} keen-slider`}>
            {[...Array(6)].map((_, index) => (
              <div
                key={index}
                className={`${styles.keenSlider__slide} keen-slider__slide`}
              >
                <FlashSaleGoods />
              </div>
            ))}
